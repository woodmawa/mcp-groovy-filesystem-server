...
- **Current version:** `0.9.51` (check `build.gradle` to confirm — this line read `0.9.39` for ten releases, because no contract watches it; `docs-markdown-fresh-fs` reads only the Baseline stack line below)
- **Baseline stack:** FS 0.9.51 / CS 1.1.11 / AW 1.30.31 - 2026-09-22 (0.9.51: `execute action=groovy` sandboxed -- `new File`/`Paths.get`/`Path.of` rewritten to confined `file()`/`path()` helpers, unconfinable stream constructors refused at compile time, `options.timeout` enforced. 0.9.50: `execute action=bash` no longer truncates a script at a double-quoted phrase with whitespace -- the script travels as base64, never argv; `mkdirs` creates missing parents on copy/move/touch/append/finalise (Groovy truth on a Path is Files.exists); `ToolSurfaceContractSpec` proves every advertised action reaches a handler; `server_transform` keys and `allowNoLocate` advertised in the schemas. Every read action de-duplicated by the per-chat served ledger, decision 206; PLAN-GATE live and tuned: command-position git/gradlew only, listing forms and shell-variable directories handled; running version stamped at claim_session; session-bootstrap 3.50; 467 FS tests (run fresh 2026-09-22 via gradle-test-summary); the CS and AW numbers are hand-maintained here and watched by nothing — see W26)
