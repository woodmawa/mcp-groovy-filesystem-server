...
> **SQL -- THE SERVER CHECKS BEFORE YOU WRITE.**
> 1. The CS `claim_session` response carries `sql_manifest`: live columns of the commonly queried CS tables plus `aw.workflow_templates` and `aw.workflow_events`. Write SQL for those tables only from it.
> 2. Any other table: a chat's first query naming its columns is refused with `SQL_UNSEEN_TABLE` and the table's live columns (even if the guess was right); retry from them, or ask first with `execute_sql table=<name>` / `execute_aw_sql table=<name>`.
> 3. A wrong column on a table you were shown is a real guess: the pre-flight refuses it with `correct_columns` and it counts in M12. Never guess twice.
> 4. Subagents share the chat's session: put the manifest and these rules in their brief.
> If a claim response has no `sql_manifest`, delivery is broken (contract `sql-manifest-delivered-at-claim`) -- say so and stop writing SQL.
>
...
- **Current version:** `0.9.66` (check `build.gradle` to confirm — this line read `0.9.39` for ten releases because no contract watched it; since CS 1.1.19 `stamp-fs-md-current` does, and `bump-baseline` sets it)
- **Baseline stack:** FS 0.9.66 / CS 1.1.40 / AW 1.30.45 - 2026-09-23 (0.9.58: telemetry honours the caller's declared `X-Mcp-Caller-Session` -- flow calls to the HTTP companion were filing as `unknown`; attribution only, PLAN-GATE still reads this process's claim. 0.9.57: `.md`/`.adoc` writes are not gated by PLAN-GATE, like scratch extensions -- doc fires never changed the next call (0/17); build.gradle stays gated. 0.9.56: the planAck schema description is one constant, `PlanGateGuard.PLAN_ACK_DESCRIPTION`, shown by execute/file_write/tools, and names the three verdicts y|c|n (CS 1.1.17). 0.9.55: replace/multi_replace refuse edits that change nothing; normalize reports the resolved path. 0.9.54: file_read grep/multi_grep hits are locate evidence; dead CS persistence of structures and listings removed. 0.9.53: `args_hash` ignores `planAck`/`intent`, so M3 can tell a changed plan from an acknowledged one. 0.9.52: ONTOLOGY-GATE exempts grep/multi_grep/structure; PLAN-GATE gates only files inside a git repo that are not scratch; gate refusals recorded as `plan_gate_fired` / `refused`, not `error`. 0.9.51: `execute action=groovy` sandboxed -- `new File`/`Paths.get`/`Path.of` rewritten to confined `file()`/`path()` helpers, unconfinable stream constructors refused at compile time, `options.timeout` enforced. 0.9.50: `execute action=bash` no longer truncates a script at a double-quoted phrase with whitespace -- the script travels as base64, never argv; `mkdirs` creates missing parents on copy/move/touch/append/finalise (Groovy truth on a Path is Files.exists); `ToolSurfaceContractSpec` proves every advertised action reaches a handler; `server_transform` keys and `allowNoLocate` advertised in the schemas. Every read action de-duplicated by the per-chat served ledger, decision 206; PLAN-GATE live and tuned: command-position git/gradlew only, listing forms and shell-variable directories handled; running version stamped at claim_session; session-bootstrap 3.50; 477 FS tests (run fresh 2026-09-22 via gradle-test-summary); the CS and AW numbers are hand-maintained here and watched by nothing — see W26)

- **Since 0.9.46, three releases that are one lesson.** **0.9.49** — `server_versions.installed_version` recorded *builds*, not installs: `copyToJarsDir` guarded on the built jar and is wired `finalizedBy`, which is Gradle's `finally` block and **runs when `installMcpbLocal` fails**, so a failed install stamped the column anyway and `WP4c`/`WP4d` read it. Now gated on the jar in the DT extension cache carrying this build's bytes, compared by hash. **0.9.47** — `execute action=groovy` had been completely dead since `e259442` (v0.7.1), roughly forty minor versions: `SecureMcpScript.getScriptOutput()` was `private` and invisible to the caller's compiled `Script1`, *and* `doGroovy` never read the output buffer, returning only the script's last expression. Two defects, the second surviving a fix to the first. It stayed dark because no spec ran the action. **0.9.48** — `file_write action=append` with absent `content` returned `success: true` and wrote nothing; and `ExecuteService` got `ExecuteServiceDispatchSpec`, the first spec in its history to call `handleToolCall` at all. **If you are about to trust that some backend works because nothing has complained, check that something actually exercises it.**
